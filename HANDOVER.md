...
> 마지막 갱신: 2026-07-21 / 최신 작업: **Windows 데스크톱 앱 신규(Tauri v2 — 트레이 상주, 트레이 메모리 168→42MB 최적화)** · 뷰어 Esc·뒤로가기 닫기 수정 · 확장 새 아이콘 · 그리기=캔버스 잉크 · 오버레이 Ctrl+휠 캔버스 줌 · 그룹 유동 크기+겹침 방지.
...
- **트레이 상주 + 메모리 절감(핵심)** — 창의 X 는 종료가 아니지만 **숨기지 않고 파괴한다**. WebView2(크로미움)는 창이 떠 있는 동안만 무겁고 `hide()` 로는 메모리가 안 빠진다(사용자 실측 트레이 168MB). → CloseRequested 를 막지 않아 창이 닫히면 WebView2 가 통째로 해제되고, **창이 0개여도 프로세스가 죽지 않게 `.run(|_,e| if ExitRequested { api.prevent_exit() })`**. 종료는 트레이 "종료"(app.exit)로만. 트레이 좌클릭·"보드 열기"는 `open_main`(창이 있으면 show, 파괴돼 있으면 `build_main_window` 로 새로 만든다 → 페이지 재로드, 세션은 WebView2 데이터 폴더에 남아 재로그인 없음). **실측: 창 열림 513MB(6 WebView2 proc) → 트레이 42MB(0 WebView2 proc), 92% 감소.**
- **~~전역 단축키~~ 제거** — 처음엔 `Ctrl+Shift+V` 전역 단축키로 담기를 넣었으나 사용자 요청으로 뺐다(`tauri-plugin-global-shortcut` 의존성·등록 코드 삭제). 담기는 **트레이 메뉴 "클립보드에서 담기"** 로만.
...
- **검증(실측)**: 실행파일 3.43MB · 로그인 화면 정상 렌더(스크린샷) · `WM_CLOSE` 후 프로세스 생존(트레이) + WebView2 프로세스 전부 종료·메모리 92%↓ · NSIS 설치파일 생성.
  - ⚠️ **클립보드 담기 실동작(카드가 실제로 생기는지)은 미검증** — 세션·네트워크가 필요해 헤드리스로 못 돌린다. §6 확인 대기.
...
- **⚠️ 확인 대기 (사용자가 직접 눈으로)**:
  - ① **데스크톱 클립보드 담기 실동작** — 앱을 켜고 링크를 복사한 뒤 트레이 우클릭 → "클립보드에서 담기" → "링크를 보드에 담았어요" 알림이 뜨고 보드에 카드가 생기는지. 세션·네트워크가 필요해 헤드리스로 검증 불가. 안 되면 볼 곳: 로그인 세션 유무, `/share` 응답, `src-tauri/src/lib.rs` 의 폴링 판정 조건. (담기 창을 열면 잠깐 WebView2 메모리가 오르나 완료 후 파괴됨)
...
- **데스크톱 앱에 아직 없는 것**(요청 범위 밖이라 안 만듦): Windows 시작 시 자동 실행, 파일 드래그해서 담기, 자동 업데이트, 코드 서명(설치 시 SmartScreen 경고가 뜬다). 자동 시작은 `tauri-plugin-autostart` 로 몇 줄이면 붙는다. 전역 단축키는 **의도적으로 뺐다**(사용자 요청) — 필요하면 `tauri-plugin-global-shortcut` 재추가.
- 데스크톱은 **원격 URL 하이브리드**라 인터넷이 없으면 폴백 화면만 뜬다(오프라인 사용 불가). `app-plan.md` P1 정적 번들이 그 전제.
- **데스크톱 메모리 성격**: 트레이 상주 시 ~42MB(WebView2 해제됨). 창을 열면 웹 페이지를 로드하는 동안 WebView2 가 붙어 수백 MB까지 오른다(브라우저 탭과 동일, 보드 카드 수에 비례) — 이건 웹뷰 앱의 본질이라 못 줄인다. 창 닫으면 다시 ~42MB.
...
- **창 닫기 = 숨김(`hide`)이 아니라 파괴.** WebView2 메모리를 트레이에서 해제하는 유일한 방법. 대신 창이 0개여도 프로세스가 죽지 않게 `RunEvent::ExitRequested → prevent_exit` 를 **반드시 유지**(빼면 창 닫는 순간 앱이 종료됨). 창은 `build_main_window` 로 다시 만든다 — 크기·url 이 바뀌면 `tauri.conf.json` 시작 창과 **양쪽 다** 맞출 것.
...
- 2026-07-21(2): 데스크톱 앱 **전역 단축키 제거**(사용자 요청) + **트레이 메모리 최적화** — 창 닫기를 숨김→파괴로 바꿔 트레이 상주 시 WebView2 를 해제(168MB→~42MB, 92%↓). `ExitRequested→prevent_exit` 로 창 없이도 트레이 유지.
