...
- 네이티브가 얹는 것만 담당:
  - **시스템 트레이 상주 + 메모리 절감** — 창의 X 는 종료가 아니지만 **숨기지 않고 파괴**한다(WebView2 메모리 해제). 창이 0개여도 `ExitRequested→prevent_exit` 로 프로세스는 살아 있고, 트레이 좌클릭·"보드 열기"로 창을 새로 만든다. **트레이 상주 시 168MB→~42MB(92%↓)**. 종료는 트레이 "종료"로만.
  - **클립보드 담기** — 트레이 메뉴 "클립보드에서 담기". (전역 단축키는 초기 구현했다가 사용자 요청으로 제거)
